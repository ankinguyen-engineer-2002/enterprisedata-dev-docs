# `Usp_CreateTableFromParquet_Straight`

_Schema: `DW_Developer` · Warehouse: `ETL_Framework` · Family: `parquet-loaders`_

_Modified: 2025-10-11 10:49:11.263000 · Code size: 6,260 chars_
...
```sql
CREATE   PROCEDURE DW_Developer.Usp_CreateTableFromParquet_Straight
    ...
    @ParquetPath         NVARCHAR(4000)   -- abfss://.../file.parquet or folder
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 1) HOLDING: bring Parquet into destination DB

        ------------------------------------------------------------

        SET @SQL = N'

IF OBJECT_ID(''' + @FullHolding + N''',''U'') IS NOT NULL DROP TABLE ' + @FullHolding + N';
...
FROM OPENROWSET(
    ...
    FORMAT = ''PARQUET''
        ...
        -- 2) FINAL (empty): copy structure from holding

        ------------------------------------------------------------

        SET @SQL = N'

IF OBJECT_ID(''' + @FullFinal + N''',''U'') IS NOT NULL DROP TABLE ' + @FullFinal + N';
...
-- … [173 more lines truncated, full body in data/06-procs-raw.json] --
```
