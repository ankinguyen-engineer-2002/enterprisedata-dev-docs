# `Usp_CreateTableFromParquet_1`

_Schema: `DW_Developer` · Warehouse: `ETL_Framework` · Family: `parquet-loaders`_

_Modified: 2025-09-22 08:50:45.887000 · Code size: 4,485 chars_

[← Procs](../README.md) · [← Logic](../../README.md) · [← Root](../../../../README.md)

## What it does

Reads from 2 sources and writes to 2 sinks. Has error handling (TRY/CATCH).
...
```sql
CREATE       PROCEDURE DW_Developer.Usp_CreateTableFromParquet_1
    @DestinationDatabase NVARCHAR(128),
    @SchemaName NVARCHAR(128),
    @TableName NVARCHAR(128),
    @ParquetPath NVARCHAR(4000)
...
BEGIN
    ...
    SET @String= 'DW_Developer.Usp_CreateTableFromParquet_1';
    SET @User = SYSTEM_USER;

    INSERT INTO [ETL_Framework].[DW_Developer].[AuditLog] (Description, DateTime, [User], Command)
   ...
   -- BEGIN
    ...
    FROM OPENROWSET(
        BULK ''' + @ParquetPath + ''',
-- … [53 more lines truncated, full body in data/06-procs-raw.json] --
```
